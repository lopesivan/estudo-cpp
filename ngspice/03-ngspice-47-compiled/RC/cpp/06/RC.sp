...
.control
    ...
    * Interseções
    ...
    meas tran tcross3 WHEN v(in)=v(out) CROSS=3
    meas tran vcross3 FIND v(in) WHEN v(in)=v(out) CROSS=3

    meas tran tcross4 WHEN v(in)=v(out) CROSS=4
    meas tran vcross4 FIND v(in) WHEN v(in)=v(out) CROSS=4

    * Passa as medidas para arquivo
    wrdata measures.dat tcross1 vcross1 tcross2 vcross2 tcross3 vcross3 tcross4 vcross4
.endc
